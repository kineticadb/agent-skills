...
[![Version](https://img.shields.io/badge/version-1.0.54-blue)](https://github.com/kineticadb/agent-skills)
...
| ----- | -------- | --------------- | ---- |
| **kinetica-code** | 💻 App developers | Python SDK (`gpudb`), REST API, data pipelines, embedded SQL | 10 |
| **kinetica-execute** | ⚡ All users | SQL analytics, graph, geospatial, time-series, visualization, security & admin — with a live dual-runtime CLI | 26 |
...
All reference files originate from a single `knowledge/` directory (26 source files, ~7,400 lines). The `build.sh` script distributes them to each skill based on its `REFS` manifest:
...
skills/kinetica-code/references/      ← 10 files (SDK + DDL/DML + delta tables)
skills/kinetica-execute/references/   ← 26 files (all domains + API + security)
```
...
├── knowledge/               # Source of truth for all reference docs (26 files)
