...
The CLI scripts are the preferred way to interact with Kinetica (they handle auth, pagination, and error formatting). However, if you need to call Kinetica's REST API directly via `curl`, see [references/curl-api-reference.md](references/curl-api-reference.md).
